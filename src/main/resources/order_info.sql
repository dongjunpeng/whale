CREATE TABLE `elephant`.`order_info` (
    ...
    `count` SMALLINT(4)  COMMENT '订单人数',
    `student_count` SMALLINT(4) COMMENT '订单人数',
    `price` BIGINT(20) COMMENT '订单总价',
    `actual_price` BIGINT(20) COMMENT '实际付款',
    `is_agreed` TINYINT(1) COMMENT '是否同意条款',
    `add_time` BIGINT(20) COMMENT '创建时间',
